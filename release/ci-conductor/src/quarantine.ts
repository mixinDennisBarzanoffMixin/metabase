...
//
// Imports here are deliberately EXTENSIONLESS (`./util`, not `./util.ts`),
// unlike the rest of the module: the e2e gate (`e2e/support/check-quarantine.ts`)
// imports this file, and e2e's tsconfig doesn't enable `allowImportingTsExtensions`.
// `contract`/`util` carry no `.ts`-specifier imports of their own, so the whole
// set e2e pulls in stays extension-clean. bun and the module's own tsc resolve
// both forms.

import type { NormalizedTest } from "./contract";
...
function finish(shouldFail: boolean, reason: string, dryRun: boolean): GateResult {
  if (shouldFail) {
...
export async function runQuarantineGate(opts: {
  ...
  if (!baseUrl) {
    ...
    return finish(true, "could not fetch the quarantine list", dryRun);
  }
